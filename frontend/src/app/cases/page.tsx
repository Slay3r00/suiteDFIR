...
export default function CaseManagementPage() {
    ...
    const handleSave = async () => {
        try {
            if (editingCase) {
                // Update
                const res = await fetch(`http://localhost:8000/api/cases/${editingCase.id}`, {
                    method: 'PUT',
                ...
                <DialogContent className="bg-[#1A1A1A] border-[#333333] text-white max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-gray-400">
                            {editingCase ? 'Update case details below.' : 'Enter the details for the new investigation.'}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsModalOpen(false)} className="border-[#333333] text-gray-400 hover:text-white hover:bg-[#333333]">
                            Cancel
                        </Button>
